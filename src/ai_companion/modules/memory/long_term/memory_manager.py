...
class MemoryManager:
    ...
    async def extract_and_store_memories(self, message: BaseMessage, chat_id: str) -> None:
        """Extract important information from a message and store in vector store, segregated by chat_id."""
        ...
        if analysis.is_important and analysis.formatted_memory:
            # Check if similar memory exists for this chat_id
            similar = self.vector_store.find_similar_memory(analysis.formatted_memory, chat_id=chat_id)
            if similar:
                # Skip storage if we already have a similar memory
                self.logger.info(f"Similar memory already exists for chat_id {chat_id}: '{analysis.formatted_memory}'")
                return

            # Store new memory with chat_id
            self.logger.info(f"Storing new memory for chat_id {chat_id}: '{analysis.formatted_memory}'")
    ...
    def get_relevant_memories(self, context: str, chat_id: str) -> List[str]:
        """Retrieve relevant memories based on the current context and chat_id."""
        memories = self.vector_store.search_memories(context, k=settings.MEMORY_TOP_K, chat_id=chat_id)
        if memories:
